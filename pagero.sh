...
if ! [ -f /tmp/pagerotest.run ]; then
    touch /tmp/pagerotest.run
else
    exit 0
fi
...
LOGPATH="/var/www/html/pagero.log"

#Incoming folder from pagero
MYPATH="/sftp/pagero/home/from_pagero/"

#Path to publish to for integration with agresso
REMOTEPATH="/mnt/erpshare/agrprod/Svefakt/In/Efakturor/"
